...
use zephyr_sdk::{
    prelude::*,
    soroban_sdk::{
        xdr::{ContractEvent, ContractEventBody, ContractEventV0, Hash, ScSymbol, ScVal},
        Address, String as SorobanString, Symbol,
    },
    EnvClient,
};

pub(crate) const FACTORY_CONTRACT_ADDRESS: [u8; 32] = [
    178, 63, 18, 76, 113, 152, 251, 31, 74, 139, 184, 239, 196, 211, 3, 205, 58, 60, 182, 44, 2,
    69, 194, 82, 254, 104, 175, 110, 187, 158, 108, 73,
];

pub(crate) const TESTNET_FACTORY_CONTRACT_ADDRESS: [u8; 32] = [
    241, 93, 189, 59, 16, 79, 135, 71, 24, 138, 236, 176, 140, 10, 244, 63, 2, 30, 160, 131, 72,
    87, 25, 115, 17, 91, 60, 82, 240, 95, 199, 186,
];
...
fn get_current_supply<T: DatabaseInteract + Common>(
    env: &EnvClient,
    event: &ContractEventV0,
    address: Address,
) -> i128 {
    let asset_topic_string =
        soroban_string_to_string(env, env.from_scval::<Address>(&event.topics[1]).to_string());
...
fn add_supply<T: DatabaseInteract + Common>(
    env: &EnvClient,
    event: &ContractEventV0,
    contract: Address,
    increase: bool,
...
pub extern "C" fn on_close() {
    ...
    let events: Vec<ContractEvent> = env
        .reader()
        .soroban_events()
        .into_iter()
        .filter(|x| pools.contains(&address_from_string(&env, x.contract_id.clone())))
        .collect();
...
pub extern "C" fn dashboard() {
    ...
    env.log()
        .debug(format!("{:?}", serde_json::to_string(&dasboard)), None);
